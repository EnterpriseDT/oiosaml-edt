    ...
    public class IDPEndpoints
    {
        ...
        private void HandleCreateIdp(string filename)
        {
            if (!_fileToEntity.ContainsKey(filename))
            {
                ...
                {
                    endp = ParseIDPEndPoint(metadataDoc);
        ...
        public static IDPEndPoint Parse(byte[] xmlMetadataBytes)
        {
            return ParseIDPEndPoint(ParseXml(xmlMetadataBytes));
        }
        ...
        {
            return ParseXml(File.ReadAllBytes(file));
        }

        private static Saml20MetadataDocument ParseXml(byte[] xmlMetadataBytes)
        {
            XmlDocument doc = LoadBytesAsXmlDocument(xmlMetadataBytes);
            ...
            } catch(Exception e)
            {
                ...
                Trace.TraceData(TraceEventType.Error, "Probably not a SAML2.0 metadata document.", e.ToString());
                return null;
            }            
        }
        ...
        public string ID
        {
            ...
            set
            {
                if (!Uri.IsWellFormedUriString(value, UriKind.RelativeOrAbsolute))
                    throw new ConfigurationErrorsException(ResourcesEx.InvalidWellformedAbsoluteUriStringFormat(value));
